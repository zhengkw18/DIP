...
parser.add_argument("--test_dir", type=str, default="./test.npy", help="Testing directory for saving prediction result. Default: ./test/test.npy")
parser.add_argument("--l2", type=float, default=10)

args = parser.parse_args()


def train_epoch(model, X, y, optimizers):  # Training Process
    model.train()
...
if __name__ == "__main__":
    ...
    if args.is_train:
        ...
        model = Model(*X_train.shape, l2=args.l2)
        model.to(device)
        optimizers = [
            optim.Adam(filter(lambda p: p.size() == (X_train.shape[0], 1), model.parameters()), lr=args.learning_rate),
            optim.Adam(filter(lambda p: p.size() != (X_train.shape[0], 1), model.parameters()), lr=args.learning_rate),
        ]
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            train_acc, train_loss = train_epoch(model, X_train, y_train, optimizers)
