...
def train_epoch(model, X, y, optimizers):  # Training Process
    ...
    for optimizer in optimizers:
        optimizer.zero_grad()
        loss, acc = model(torch.from_numpy(X).to(device), torch.from_numpy(y).to(device), Lg=Lg)
        loss.backward()
        optimizer.step()
        Lg = False
...
if __name__ == "__main__":
    ...
    if args.is_train:
        ...
        model = Model(X_train.shape[1], drop_rate=args.drop_rate, l1=args.lambda1)
        model.to(device)
        optimizers = [
            optim.Adam(filter(lambda p: p.size() == (X_train.shape[1], 1), model.parameters()), lr=args.learning_rate),
            optim.Adam(filter(lambda p: p.size() != (X_train.shape[1], 1), model.parameters()), lr=args.learning_rate),
        ]
